...
DWORD ByteRez;
        // Получить в byteRez кол-во памяти нужное под список стуктур принтеров
//EnumPrinters(PRINTER_ENUM_LOCAL,NULL,1,NULL,NULL,&ByteRez,&PrintersCount);    // Установленные на компьютер принтеры
//EnumPrinters(PRINTER_ENUM_NAME,NULL,1,NULL,NULL,&ByteRez,&PrintersCount);       // Провайдеры печати
EnumPrinters(PRINTER_ENUM_LOCAL||PRINTER_ENUM_CONNECTIONS,NULL,1,NULL,NULL,&ByteRez,&PrintersCount);       // Провайдеры печати
ShowMessage(IntToStr(PrintersCount));
        // Выделить нужную память
//PRINTER_INFO_1* printers = (PRINTER_INFO_1*)malloc(ByteRez);
PRINTER_INFO_1* printers = (PRINTER_INFO_1*)malloc(ByteRez);
        // Повторным вызовом получить список структур принтеров
//bool Rez = EnumPrinters(PRINTER_ENUM_LOCAL,NULL,1,(LPBYTE)printers,ByteRez,&ByteRez,&PrintersCount);
//bool Rez = EnumPrinters(PRINTER_ENUM_NAME,NULL,1,(LPBYTE)printers,ByteRez,&ByteRez,&PrintersCount);
bool Rez = EnumPrinters(PRINTER_ENUM_NAME||PRINTER_ENUM_CONNECTIONS,NULL,1,(LPBYTE)printers,ByteRez,&ByteRez,&PrintersCount);
...
//        TreeView->Items->Add(NULL,"Локальные принтеры");
        TreeView->Items->Add(NULL,"принтеры");
//        int TempItem = 1;
...
//                else {
//                        TreeView->Items->AddChild(TreeView->Items->Item[0],(AnsiString)printers[i].pName);
                        if(printers[i].Flags==PRINTER_ENUM_CONTAINER) ShowMessage("LL");
                        TreeView->Items->AddChild(TreeView->Items->Item[0],(AnsiString)printers[i].pName);
//                        TempItem++;
//                        }
